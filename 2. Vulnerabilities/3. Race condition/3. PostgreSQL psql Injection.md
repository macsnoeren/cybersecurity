# PostgreSQL psql Injection (CVE-2025-1094)

PostgreSQL has issued a security announcement and fixed the PostgreSQL SQL injection 
vulnerability (CVE-2025-1094), with a CVSS score of 8.1. Since the psql tool of PostgreSQL 
is used to detect invalid UTF-8 characters (such as hax\xC0′; ! id #), resulting in command 
injection vulnerabilities.

[https://nsfocusglobal.com/postgresql-sql-injection-vulnerability-cve-2025-1094/](https://nsfocusglobal.com/postgresql-sql-injection-vulnerability-cve-2025-1094/)
...
## Steps
1. Explain PostgreSQL client architecture and UTF-8 handling
2. Show the race condition between validation and execution
3. Demonstrate file modification during the race window
4. Show command injection through UTF-8 bypass
5. Implement atomic operations as mitigation
6. Discuss time-of-check-to-time-of-use (TOCTOU) vulnerabilities

## Output
```shell
=== PostgreSQL psql Race Condition Simulation (CVE-2025-1094) ===
Real-world impact: Command injection in PostgreSQL client tools

================================================================================
🚨 VULNERABLE VERSION - Race Condition Exploit
================================================================================

📋 Test 2: Race Condition Attack Simulation
🔗 Thread VICTIM: Starting psql execution...
🎯 Attacker ATTACKER: Attempting to modify temp file during validation...
📝 Thread VICTIM: Writing SQL to /tmp/tmpXYZ.sql
🔍 Thread VICTIM: Validating UTF-8 encoding...
💀 Attacker ATTACKER: Injected command execution payload!
🚀 Thread VICTIM: Executing psql command...
💀 Thread VICTIM: COMMAND INJECTION DETECTED!
🎯 Malicious command would execute: whoami

📋 Test 3: Direct UTF-8 Command Injection
⚠️  Thread DIRECT_ATTACK: Invalid UTF-8 detected, but processing continues...
💀 Thread DIRECT_ATTACK: COMMAND INJECTION DETECTED!
🎯 Malicious command would execute: whoami

================================================================================
✅ SAFE VERSION - Atomic Operations and Validation
================================================================================

📊 Safe Implementation Results:
  Thread SAFE_1: BLOCKED
  Thread SAFE_2: BLOCKED
  Thread SAFE_3: BLOCKED

📋 CVE-2025-1094 TECHNICAL DETAILS
================================================================================
Vulnerability: PostgreSQL psql UTF-8 validation race condition
CVSS Score: 8.1 (High)
Impact: Arbitrary command execution with psql user privileges
```
